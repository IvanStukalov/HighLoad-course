...
Для хранения медиа (`photo`, `video`, `voice`) выберем CEPH. В отличие от облачных хранилищ данных, не требует
...
Также копии таблиц `user`, `message`, `comment`, `post` можно положить в ХД ElasticSearch (в логической схеме `user_search`, `message_search`, `comment_search`, `post_search`). Оно предоставляет механизм
...
Лента пользователя будет состоять из постов друзей, постов, которые лайкнули друзья пользователя, рекламы, а также самых
популярных постов (с самым большим количеством лайков/комментариев/просмотров) в соцсети. В идеале стоит перемешивать
эти категории.

Лента будет приходить в виде блоков постов (каждый блок состоит из двух-трех постов одной темы - посты друзей, посты,
лайкнутые друзьями, популярные посты, рекламные посты) с комментариями.
...
Для эффективного поиска друзей, поиска по постам, сообщениям и комментариям будем использовать движок ElasticSearch. Это
...
Поиск по постам будет также представлять из себя блочный алгоритм, по подобию ленты

Поиск по людям будет отдавать приоритет людям с наибольшим количеством общих друзей.

Поиск по сообщениям и комментариям будет отдавать приоритет самым свежим сообщениям/комментариям.
...
memcached) - поддержку транзакций, очереди, а также поддерживает master-slave репликацию и кластеризацию с версии 3.0.
...
Брокер сообщений представляет собой тип построения архитектуры, при котором элементы системы «общаются» друг с другом с
помощью посредника. Благодаря его работе происходит снятие нагрузки с веб-сервисов, так как им не приходится заниматься
пересылкой сообщений: всю сопутствующую этому процессу работу он берёт на себя.

В качестве брокера будет выступать Kafka
...
Для сжатия и передачи видео, аудио и фото в базе данных можно использовать следующие протоколы и методы:

Сжатие с потерями: Это метод, который основывается на несовершенстве человеческого слуха и зрения при восприятии звуковой и изобразительной информации. Он использует эффект маскировки, который зависит от спектральных и временных характеристик маскируемого и маскирующего сигналов. Наилучшим методом кодирования звука, учитывающим эффект маскирования, оказывается полосное кодирование.
...
Для передачи данных больших размеров, таких как видео или аудио, можно использовать чанковую передачу. Это метод передачи данных, при котором поток данных разбивается на непересекающиеся "части" или "чанки". Эти части передаются и принимаются независимо друг от друга. 
...
* Дневная аудитория - 54 млн. Каждый человек заходит примерно 5 раз в день.
* Умножим на пиковый коэффициент 1.67.

| Сервис    | Пиковый RPS | CPU  |  RAM   |     Net     |
|-----------|:------------|:----:|:------:|:-----------:|
| Auth      | 5000        |  50  |  5 Гб  |  5 Мбит/с   |
| Profile   | 6           | 0,06 |  6 Мб  | 2,3 Мбит/с  |
| Messanger | 600000      | 6000 | 600 Гб | 1600 Гбит/с |
| Post      | 35000       | 350  | 35 Гб  | 5500 Гбит/с |
| Comment   | 120000      | 1200 | 120 Гб | 1,6 Гбит/с  |
| proxy     | 760000      | 1500 | 15 Гб  | 7000 Гбит/с |
...
| Messanger | own     | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s                                          | 256   | 40  | 10 000     | 167       |
| Post      | own     | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s                                          | 64    | 110 | 10 000     | 167       |
| Comment   | own     | 2x6434/4x4GB/1xNVMe256Gb/2x1Gb/s                                          | 64    | 24  | 6 000      | 80        |
| proxy     | own     | 2x6338/4x8GB/2xNVMe4T/2x25Gb/s                                            | 64    | 150 | 8 000      | 96        |
...
4500*8 + 4500*2 + 10000*40 + 10000*110 + 6000*24 + 8000*150 = 2 889 000 $

С момента создания ВК прошло 17 лет.

Если делать замену через ~10 лет, то стоимость выйдет 5778000 $ 

**Аренда, $**

С момента создания ВК прошло 17 лет.

(50*8 + 50*2 + 167*40 + 167*110 + 80*24 + 96*150) * 12 * 17 = 41 870 * 12 * 17 = 8 541 480 $
