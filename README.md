...
Также в Redis положим таблицы `conversation` и `friednship`, поскольку они редко изменяются, но к ним идет частое 
обращение.

Для хранения медиа (`photo`, `video`, `voice`) будем использовать хорошо масштабирующееся надежное Amazon S3.

Для оставшихся таблиц `user`, `message`, `comment`, `post` можно взять ХД ElasticSearch. Оно предоставляет механизм для поиска, анализа и хранения данных в реальном времени. Elasticsearch используется для индексации и поиска различных типов информации, например, текстовых данных.
...
`friendship`: hash по friendship_id, hash по first_user, hash по second_user

`user_session`: hash по session_id, hash по user_id

`conversation`: hash по conversation_id, hash по users[] для поиска диалога по user_id

`message`: hash по message_id, hash по conversation_id, btree по creation_time для сортировки

`post`: hash по post_id, hash по creator_id, btree по creation_time для сортировки

`comment`: hash по comment_id, hash по post_id, btree по creation_time для сортировки
...
`friendship`: по friedndship_id

`user_session`: по session_id

`conversation`: по conversation_id
...
Для более эффективного построения ленты введем еще одну сущность - `feed`

![](./img/logical_scheme_feed.png)

Пользователь намного чаще читает ленту, чем сам выпускает посты, поэтому каждому пользователю создадим свою сущность 
Ленты, которая будет обновляться когда любой друг пользователя выпустит пост.

Когда сам пользователь перейдет на страницу Ленты, мы просто отдадим ему уже готовую ленту.

Лента пользователя будет состоять из постов друзей, постов, которые лайкнули друзья пользователя, а также самых популярных постов (с самым большим количеством лайков/комментариев/просмотров) в соцсети. В идеале стоит перемешивать эти категории.

Посты в ленте будут сортироваться по популярности, дате и по флагу "Просмотрено". Посты, уже просмотренные пользователем, будут находиться внизу ленты.

### Поиск

Для эффективного поиска друзей, поиска по постам, сообщениям будем использовать движок ElasticSearch. Это мощный и гибкий поисковый и аналитический движок, который основан на технологии поиска по тексту.
